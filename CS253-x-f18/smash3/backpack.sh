...
#
# DESCRIPTION
#   This script grades the CS253 smash3 project, awarding partial
...
rm -fr $exe $test.out *.o *.d $errMsg diff.out $testDir *~
...
#Verify smash uses the fork, exec and wait APIs to execute external commands------
logMsg $LINENO "Note:  Verifying use of fork/exec/wait APIs"
cat *.c >$tmpFile
deductnGrep $LINENO "fork" $tmpFile       $halfLetterGrade
deductnGrep $LINENO "exec[a-z]" $tmpFile  $halfLetterGrade
deductnGrep $LINENO "wait" $tmpFile       $halfLetterGrade
